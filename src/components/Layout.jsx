...
export function Shell({ children, step }) { return <main className="shell"><div className="sparkles">💗　✨　💕　⭐　💗　✨</div>{step && <div className="progress"><span>Step {step}/7</span><div><i style={{width:`${step / 6 * 100}%`}} /></div></div>}<motion.section className="card" initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }} transition={{ duration: .35 }}>{children}</motion.section></main>; }
