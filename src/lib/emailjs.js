...
export const sendConfirmationEmail = async ({ date, time, foods }) => {
  ...
  return emailjs.send(service, template, { subject: 'Your Best Friend Accepted!', selected_date: date, selected_time: time, selected_foods: foods.join(', '), timestamp: new Date().toLocaleString(), visitor_ip: ip, visitor_location: location, to_email: to }, key);
};
